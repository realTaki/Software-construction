...
public class Filter {
	...
	public static List<Tweet> writtenBy(List<Tweet> tweets, String username) {
		...
		for (Tweet aTweet : tweets) {
			if (!aTweet.getAuthor().equals(username))
				tweet.remove(aTweet);
		}
	...
	public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
		List<Tweet> tweet = new ArrayList<Tweet>(tweets);
		for (Tweet aTweet : tweets) {
			if (aTweet.getTimestamp().isAfter(timespan.getEnd())||aTweet.getTimestamp().isBefore(timespan.getStart()))
				tweet.remove(aTweet);
		}
	...
	public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
		...
		for (Tweet aTweet : tweets) {
			...
			for (String word :words) {
				if (aTweet.getText().indexOf(word)==-1) {
					flag=true;
					break;
				}
			}
